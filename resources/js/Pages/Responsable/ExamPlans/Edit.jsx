...
export default function ExamPlansEdit({ 
    examPlan = {}, 
    groups = [], 
    modules = [], 
    teachers = [], 
    ...
    // Auto-set end time when duration changes
    useEffect(() => {
        if (data.start_time && data.duration_minutes) {
            const start = new Date(`2000-01-01T${data.start_time}`);
            const end = new Date(start.getTime() + data.duration_minutes * 60 * 1000);
            const endTime = end.toTimeString().slice(0, 5); // Get HH:MM format
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('responsable.exam-plans.update', examPlan.id), {
            onSuccess: () => {
    ...
    return (
        <AuthenticatedLayout header="Edit Exam Plan - Responsible">
            <Head title="Edit Exam Plan - Responsible" />
            ...
            <div className="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 flex items-center">
                                <Edit2 className="h-8 w-8 mr-3 text-blue-600" />
                                Edit Exam Plan
                            </h1>
                            <p className="text-gray-600 mt-1">Update exam plan details for validation</p>
                        </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    <div className="flex items-start">
                        ...
                        <div>
                            <h3 className="text-sm font-medium text-blue-800">Current Plan Information</h3>
                            <p className="text-sm text-blue-700 mt-1">
                                You are editing the exam plan for <strong>{examPlan.module_name}</strong> 
                                {examPlan.group_name && ` in group <strong>${examPlan.group_name}</strong>`}.
                                All changes will be submitted for validation by the Head Department.
                            </p>
                        </div>
                    </div>
                </div>
